...
def get_unspent(address, testnet):
	...
	try:
		response = requests.get(f'https://chain.so/api/v2/get_tx_unspent/{network}/{address}').json()
	except:
		print("(sleeping for 1 minute, reached rate limit for calls to blockchain api)")
		time.sleep(60)
		response = requests.get(f'https://chain.so/api/v2/get_tx_unspent/{network}/{address}').json()
...
def get_spent(address, testnet):
	...
	network = 'BTCTEST' if testnet else 'BTC'
	try:
		response = requests.get(f'https://chain.so/api/v2/get_tx_spent/{network}/{address}').json()
	except:
		print("(sleeping for 1 minute, reached rate limit for calls to blockchain api)")
		time.sleep(60)
		response = requests.get(f'https://chain.so/api/v2/get_tx_spent/{network}/{address}').json()
	stxos = response['data']['txs']
	clean_stxos = [{'value': int(float(i['value'])*100000000), 'txid': i['txid'], 'confirmations': i['confirmations']} for i in stxos]
	return clean_stxos
